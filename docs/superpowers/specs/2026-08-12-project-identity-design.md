...
**Status:** designed, not started. Blocked on #114 landing.
...
`#114` lands; rebase. The native-layer bundle path fix (spawned separately) now
also needs the bundle *name*, since #114 renamed `app.app` to `Flutterware.app`
— it should read the name rather than hardcode either spelling.
