...
class FrameCapture {
  ...
  /// Arms [count] sequential captures and hands back one future per frame, in
  /// the order the guest will present them.
  ///
  /// The names are predictable — `<prefix><n>.rawframe`, the host's own
  /// counter — so every completer can be registered before a single frame
  /// exists, and each resolves the moment its file is acked. That is what lets
  /// the caller read and encode frame 3 while the guest is still drawing frame
  /// 40: the two halves of a render stop taking turns.
  ///
  /// Nothing is scheduled here. Something inside the guest — the motion
  /// registry's render loop — is what draws the frames this writes, and this
  /// only says where to put them.
  ///
  /// No future carries a timeout, deliberately: a sequence of two thousand
  ...
  Future<List<Future<RawFrame>>> captureSequence({
    ...
    for (var index = 0; index < count; index++) {
      var path = '$prefix$index.rawframe';
      var file = File(path);
      // A leftover from an abandoned render would otherwise be read as this
      // one's frame, and it would be a picture of the right size and the wrong
      // moment.
      if (file.existsSync()) file.deleteSync();
      var done = _pending[path] = Completer<Object?>();
      futures.add(
        done.future.then((failure) {
  ...
  /// Disarms a sequence and fails whatever is still outstanding of it.
  ///
  /// Prefix-scoped rather than [failAll], so ending one render does not take
  /// down a screenshot somebody asked for at the same moment.
  void endSequence(String prefix, {Object? because}) {
    unawaited(send(const CaptureSequenceMessage(prefix: '', count: 0)));
    for (var key in _pending.keys.toList()) {
      if (!key.startsWith(prefix)) continue;
      _pending
          .remove(key)
          ?.complete(
            because ??
                StateError('the render ended before this frame was written'),
          );
    }
  }
