...
/// `permissions` — what each package *declares*, read from what it ships.
///
/// The declared third of a permission, and the only third that needs no
/// device, no run and no build. Held and observed state arrive in later
/// phases; see the design doc named in `run/permissions.dart`.
@JsonSerializable(
  explicitToJson: true,
  includeIfNull: false,
  createFactory: false,
)
class RunPermissionsResult implements PluginResult {
  RunPermissionsResult({required this.packages, this.note});

  final List<RunPermissionPackage> packages;

  /// Said out loud when there is nothing to report, rather than left for the
  /// caller to infer from an empty array.
  final String? note;

  @override
  Map<String, Object?> toJson() => _$RunPermissionsResultToJson(this);
}
...
class RunPermissionPackage {
  ...
  /// Which platform directories this package actually has — `android`, `ios`,
  /// `macos`. An app with no `ios/` has not forgotten its usage descriptions.
  final List<String> platforms;

  final List<RunPermissionRow> rows;

  /// What disagrees. Named `findings` rather than `lints` because they travel
  /// to callers who never asked for a linter.
  final List<RunPermissionFinding> findings;

  /// The files actually read, package-relative. A report that names its
  /// sources can be checked; one that does not has to be trusted.
  final List<String> sources;

  /// True when an Android merged manifest was read, so dependencies'
  /// permissions are included. False means this is the app's own list, which
  /// grows once after the first build.
  final bool merged;

  /// The device the `held` column was read from, when one was asked for.
  final String? device;

  /// The application id the host-side read used — **not** the Dart package
  /// name. Reported so a wrong one is correctable rather than mysterious.
  final String? appId;

  /// Where [appId] came from: the merged manifest, a Gradle guess, the Xcode
  /// project.
  final String? appIdSource;

  /// Why there is no held state, when a device was asked and could not answer.
  /// Present exactly when the read failed, so rows without a `held` value are
  /// never ambiguous between "undetermined" and "nobody could say".
  final String? heldUnavailable;

  /// Why particular rows read `unknown` on this device — iOS location and
  /// notifications live outside the store this reads. Distinct from
  /// [heldUnavailable], which is about the device as a whole.
  final List<String> heldNotes;

  final String? note;

  Map<String, Object?> toJson() => _$RunPermissionPackageToJson(this);
}
...
class RunPermissionRow {
  ...
  /// `camera` — or the raw identifier, when nothing in the catalogue claims
  /// it. Declared things are never dropped for being unrecognised.
  final String capability;

  final String label;

  /// Which platforms declare this capability.
  final List<String> platforms;

  final List<RunPermissionDeclaration> declarations;

  /// False when this is reported under its raw identifier because nothing
  /// named it — `INTERNET` and Flutter's own generated receiver permission are
  /// the standing examples. Still declared, still listed, just not the point.
  final bool known;

  /// What the OS holds: `granted`, `denied`, `deniedForever`, `undetermined`
  /// or `unknown`. Absent when no device was asked, or when the device could
  /// not answer — `heldUnavailable` on the package says which.
  final String? held;

  /// What the **app itself** believes, when it wired a `PermissionAdapter`.
  /// Absent when it did not, which is most apps. Where this disagrees with
  /// [held] the app has cached a status across a change — a real bug that
  /// nothing outside the process can see.
  final String? observed;

  Map<String, Object?> toJson() => _$RunPermissionRowToJson(this);
}
...
class RunPermissionDeclaration {
  ...
  /// entitlement key — what you would actually type to act on this.
  ...
  Map<String, Object?> toJson() => _$RunPermissionDeclarationToJson(this);
}
...
class RunPermissionFinding {
  ...
  /// `problem`, `warning` or `note`. Nothing here stops a build.
  final String severity;

  /// Written to be read alone, because it travels to `fw` and MCP where there
  /// is no row beside it.
  ...
  Map<String, Object?> toJson() => _$RunPermissionFindingToJson(this);
}

/// `permissionMatrix` — the same app, the same screen, once per profile.
///
/// The one result in this file that costs minutes rather than milliseconds,
/// and the fields say where they went: every cell carries its own `ms`, and a
/// run that stopped early says so in [note] rather than returning a short list
/// that looks complete.
@JsonSerializable(
  explicitToJson: true,
  includeIfNull: false,
...
class RunPermissionMatrixResult implements PluginResult, ProducesArtifacts {
  RunPermissionMatrixResult({
    required this.device,
  ...
  /// The route each cell was driven to before it was photographed, when one
  /// was asked for. Without it every cell shows the app's first screen, which
  /// is often not where a permission is read.
  final String? route;

  /// In the order they were run — which is the caller's order, not the
  /// enum's.
  final List<RunPermissionMatrixCell> cells;

  /// What the grid cannot show: a run cut short, a wish that will undo the
  /// last cell, every screen having come out identical.
  final String? note;

  @override
  @JsonKey(includeToJson: false)
  List<Artifact> get artifacts => [
    for (var cell in cells) ?cell.screenshotArtifact,
  ];

  @override
  Map<String, Object?> toJson() => _$RunPermissionMatrixResultToJson(this);
}

/// One profile's launch, and what the app looked like under it.
@JsonSerializable(
  explicitToJson: true,
  includeIfNull: false,
  createFactory: false,
)
class RunPermissionMatrixCell {
  ...
  /// The profile id — `first-run`, `granted`.
  final String profile;

  final String label;

  /// The launch came up and the screen was observed. False leaves the rest of
  /// the cell empty and puts the reason in [error]; the matrix carries the
  /// hole rather than dropping the cell, because a profile that cannot be
  /// launched is a result.
  final bool ok;

  /// What the OS held after the profile was applied, capability → state.
  /// **The read-back**, not the profile echoed: a device that refused half of
  /// it says so here, in the cell that ran under it.
  final Map<String, String> held;

  /// What the app itself reported, for an app with a `PermissionAdapter`
  /// wired. Empty for every other app, which is most of them.
  final Map<String, String> observed;

  /// Where this cell's PNG was written — the run's journal directory, so the
  /// cell is also a step somebody can scrub back to.
  final String? screenshot;

  /// Every text on screen after the settle.
  final List<String>? texts;

  /// Texts this cell had and the **first** cell did not. The compact form of
  /// "what did denying it change", and empty for the first cell by
  /// definition.
  final List<String> added;

  final String? error;

  /// Anything the platform could not do for this cell — a state iOS does not
  /// have, a capability with nothing behind it.
  final String? note;

  /// Apply, launch, drive and observe, together. The honest cost of one cell.
  final int? ms;

  @JsonKey(includeToJson: false)
  final Artifact? screenshotArtifact;

  Map<String, Object?> toJson() => _$RunPermissionMatrixCellToJson(this);
}
