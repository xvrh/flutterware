...
import '../../run/permission_matrix_view.dart';
...
class _NewRunPageState extends State<_NewRunPage> {
  ...
  /// What this entry point's package declares, read from its manifests.
  ///
  /// Cached per package because it walks `build/` for merged manifests and
  /// reads several files, and `build` runs on every keystroke in the flavor
  /// field.
  ///
  /// **What invalidates it is a build**, not an edit: the documented "the list
  /// grows once" happens when the merged manifest first appears. There is
  /// exactly one build that finishes while this page is still on screen — the
  /// matrix, which runs four — so [_forgetDeclarations] runs when its dialog
  /// closes, and on an entry-point change. Start needs nothing: it navigates
  /// to the run it launched, and coming back builds this state afresh.
  ///
  /// A build started *elsewhere* is not noticed until this page is rebuilt.
  /// Said plainly because an earlier version of this comment promised a
  /// clearing that no code performed.
  final _declarations = <String, PermissionDeclarations>{};

  void _forgetDeclarations() => _declarations.clear();

  PermissionDeclarations? get _permissions {
    var choice = _entry;
    if (choice == null) return null;
    return _declarations[choice.package] ??= readDeclarations(
      _core.host.workspace.absolutePathOf(choice.package),
    );
  }

  /// What the selected device already holds for the selected app.
  ///
  /// Keyed by both, because it is a property of the pair: the same app holds
  /// different things on the emulator and on the simulator, and that is the
  /// whole point of showing it beside a device picker.
  ///
  /// A future rather than a value: this shells out to `adb` or `sqlite3`, and
  /// `build` runs on every keystroke in the fields above.
  final _held = <String, Future<HeldPermissions>>{};

  PermissionProfile? _applyingProfile;

  /// Bumped after every write, so the re-read cannot show the pre-write answer
  /// while it is in flight — the same staleness the device key guards against.
  int _heldRevision = 0;

  /// Which of the three platforms the chosen device is, when it is one this
  /// reads permissions for.
  ///
  /// Null for the web and for anything the device list does not name, and then
  /// the rows fall back to saying where the capability is *declared* — which is
  /// all that can be said without a platform to be specific about.
  PermissionPlatform? get _selectedPlatform {
    var id = _device;
    if (id == null) return null;
    for (var device in _core.devices) {
      if (device.id != id) continue;
      return switch (device.platformType) {
  ...
  /// The one spelling of "which app on which device", written once.
  ///
  /// It was written three times before, and one copy had picked up a NUL as
  /// its separator — this repo uses one deliberately in `FlagMemory` and
  /// `closure.dart`, and it is invisible in an editor. The write stored its
  /// read-back under one spelling while the reader looked up another, so the
  /// column kept the pre-write answer for ever. (It also made `grep` treat the
  /// whole file as binary and report no matches, which cost more than the bug.)
  String get _heldCacheKey => '${_entry?.package}|$_device';

  /// [_heldCacheKey] plus the revision — identifies *which read*, so a new one
  /// replaces the FutureBuilder rather than updating it.
  String get _heldKey => '$_heldCacheKey|$_heldRevision';

  /// Applies a profile to the chosen device, then re-reads.
  ///
  /// The cache entry is dropped rather than updated, so the column comes from
  /// the same read path as everything else. A write that reported its own
  /// result into the UI would be the echo this design refuses.
  Future<void> _applyProfile(PermissionProfile profile) async {
    var choice = _entry;
    var device = _device;
    if (choice == null || device == null) return;
    setState(() => _applyingProfile = profile);
    try {
      // **The write's own read-back becomes the column.** Kicking off a second
      // read here instead raced the first one and left the pre-write answer on
      // screen — and it was redundant anyway, since a write already reads back
      // by contract. One read, one answer, no window where the two disagree.
      var written = await _core.applyPermissionProfile(
      ...
      if (!mounted) return;
      setState(() {
        _applyingProfile = null;
        _error = null;
        _heldRevision++;
        _held[_heldCacheKey] = Future.value(written.after);
      });
    } on Object catch (e) {
      // **Shown, not rethrown.** This runs from an `onTap` that drops the
      // future, so a rethrow reached nothing but the console: the chip stopped
      // spinning, the column kept its pre-write value, and the page said the
      // write had happened. Same treatment as a failed launch, one field down.
      if (mounted) {
        setState(() {
          _applyingProfile = null;
          _error = 'Could not apply ${profile.label}: $e';
        });
      }
    }
  }

  /// Opens the matrix, and drops the columns afterwards.
  ///
  /// Dropping them is not housekeeping: the matrix leaves the device under its
  /// *last* cell, so the held and observed columns on this page describe a
  /// state that stopped being true minutes ago. A stale permission column is
  /// the one thing this whole field exists to prevent.
  Future<void> _compare() async {
    var choice = _entry;
    var device = _device;
    if (choice == null || device == null) return;
    var (:flavor, :knobs) = _choices;
    await showPermissionMatrixDialog(
      context,
      core: _core,
      package: choice.package,
      entry: choice.entry,
      device: device,
      // The same build Start would make. Only the permissions vary across the
      // cells; everything else on this page has to travel with them.
      flavor: flavor,
      knobs: knobs,
      deviceLabel:
          _core.devices
    ...
    setState(() {
      _heldRevision++;
      _held.remove(_heldCacheKey);
      _observed.remove(_heldCacheKey);
      // Four builds happened behind that dialog, so the merged manifest may
      // exist now where it did not before.
      _forgetDeclarations();
    });
  }

  /// What the app running on the chosen device believes about itself.
  ///
  /// A separate future from the held read: it needs a *running* app with an
  /// adapter wired, which is a much narrower thing than a connected device,
  /// and an app that offers nothing must not stop the held column arriving.
  Future<Map<String, HeldState>>? get _observedPermissions {
    var device = _device;
    if (device == null) return null;
    return _observed[_heldCacheKey] ??= _core.observedPermissions(
      device: device,
    );
  }

  final _observed = <String, Future<Map<String, HeldState>>>{};

  Future<HeldPermissions>? get _heldPermissions {
    var choice = _entry;
    var device = _device;
    if (choice == null || device == null) return null;
    return _held[_heldCacheKey] ??= _core.heldPermissions(
      package: choice.package,
      device: device,
    );
  }
...
/// twice.
/// What this app asks for, before it has been asked for anything.
///
/// Read-only in this phase, deliberately: there is nothing here to set until
/// there is a device to set it on. What it is for now is the disagreement
/// report — the part that works with no device, no run and no build, and the
/// part that catches the mistakes which otherwise surface as "the prompt never
/// appears" or a store rejection.
class _PermissionsField extends StatelessWidget {
  const _PermissionsField({
    required this.declarations,
    required this.heldKey,
    this.held,
    this.observed,
    this.platform,
    this.onApply,
  ...
  /// What the app itself believes, once it answers.
  ///
  /// Empty for the overwhelming majority of apps, which wire no adapter — and
  /// then the column is simply absent rather than a row of dashes implying the
  /// app was asked and said nothing.
  final Future<Map<String, HeldState>>? observed;

  /// The platform of the chosen device, when there is one.
  ///
  /// It decides what the middle column is *about*. With no device the only
  /// honest thing to say is where a capability is declared — `Android · iOS`.
  /// With a device chosen that reads as if it describes the run, which it does
  /// not: on an emulator the `iOS` half is unactionable. So the column becomes
  /// this platform's own identifier — the thing you would type into a
  /// `pm grant` — and says plainly when this platform does not declare the
  /// capability at all, which is the case where the held column can only ever
  /// be blank.
  final PermissionPlatform? platform;

  final PermissionDeclarations declarations;

  /// What the chosen device already holds, once it answers. Null when no
  /// device is chosen — and then the column is simply absent rather than full
  /// of dashes promising a state nobody asked for.
  final Future<HeldPermissions>? held;

  /// Null when there is no device to write to, and then no chips are offered.
  final ValueChanged<PermissionProfile>? onApply;

  /// Opens the matrix — one launch per profile. Gated on the same device the
  /// chips are: there is nothing to compare on a device nothing can be written
  /// to.
  final VoidCallback? onCompare;

  final PermissionProfile? busy;

  /// Identifies *which* read [held] is. Changing it replaces the builder
  /// rather than updating it.
  ///
  /// Not decoration. `FutureBuilder` keeps `snapshot.data` when its future
  /// changes — it only moves the connection state — so switching device
  /// rendered the previous device's permissions under the new device's name
  /// until the new read landed. Chrome showed the emulator's rows, with the
  /// profile chips that Chrome cannot offer. A key makes it a different
  /// element, which cannot carry a stale answer.
  final String heldKey;

  @override
  Widget build(BuildContext context) {
    if (held == null) return _body(context, null, const {});
    return FutureBuilder<HeldPermissions>(
      key: ValueKey(heldKey),
      future: held,
      builder: (context, held) => FutureBuilder<Map<String, HeldState>>(
        // Keyed alike, for the same reason: a snapshot that outlived its
        // request would put one device's answer under another's name.
        key: ValueKey('observed:$heldKey'),
        future: observed,
        builder: (context, seen) =>
            _body(context, held.data, seen.data ?? const {}),
      ),
    );
  }
  ...
  Widget _body(
    ...
    HeldPermissions? state,
    Map<String, HeldState> observed,
  ) {
    // Problems first and in full. A finding is the reason this field exists,
    // so it goes above the list rather than as an annotation somebody has to
    // find by scrolling.
    var findings = [...declarations.lints]
      ..sort((a, b) => a.severity.index.compareTo(b.severity.index));
    // Named capabilities are the list; everything else is a footnote. Both are
    // reported — see [PermissionRow.known] for what looking at this field once
    // taught about the difference.
    var named = [
      for (var row in declarations.rows)
        if (row.known) row,
    ];
    var raw = [
      for (var row in declarations.rows)
    ...
    var count = declarations.sources.length;
    var footnote = [
      'Read from $count ${count == 1 ? 'file' : 'files'}',
      if (raw.isNotEmpty)
        'also declares ${[for (var row in raw) row.label].join(', ')}',
    ].join(' · ');
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        for (var finding in findings) ...[
          _PermissionFinding(finding: finding),
          const Gap(FwSpacing.sm),
        ],
        // Only where there is something to write to. On macOS, a physical
        // iPhone or the web these would be buttons that cannot work, and an
        // offer the platform will refuse is worse than no offer.
        if (onApply case var apply? when state?.available ?? false) ...[
          Wrap(
            ...
            children: [
              for (var profile in PermissionProfile.values)
                _ProfileChip(
                  label: profile.label,
                  busy: busy == profile,
                  onTap: busy == null ? () => apply(profile) : null,
                ),
              // Last, and drawn as a different kind of thing: the four above
              // are one write each, this one is a relaunch per profile. An
              // icon is the cheapest way to say "this is not a fifth profile"
              // in a row of chips.
              if (onCompare case var compare?)
                _ProfileChip(
                  label: 'Compare…',
                  icon: Icons.grid_view_outlined,
                  onTap: busy == null ? compare : null,
                ),
            ],
          ),
          const Gap(FwSpacing.sm),
        ...
        DecoratedBox(
          ...
          child: Column(
            children: [
              for (var (index, row) in named.indexed)
                DecoratedBox(
                  decoration: BoxDecoration(
                    border: index == 0
                        ? null
                  ...
                  child: Padding(
                    ...
                    child: Row(
                      children: [
                        Expanded(
                          child: Text(row.label, style: context.type.body),
                        ),
                        // Fixed widths, not flex. Sharing the row with the
                        // label left the held column landing in a different
                        // place on every row, which reads as noise in a list
                        // whose whole job is to be scanned down.
                        SizedBox(
                          width: 216,
                          child: _RowSubject(row: row, platform: platform),
                        ),
                        if (state != null && state.available) ...[
                          ...
                          SizedBox(
                            width: 96,
                            child: _HeldCell(
                              state: state.byCapability[row.capability],
                            ),
                          ),
                        ],
                        if (observed.isNotEmpty) ...[
                          ...
                          SizedBox(
                            width: 96,
                            child: _HeldCell(
                              state: observed[row.capability],
                              disagrees:
                                  state != null &&
                                  _disagree(
                                    state.byCapability[row.capability],
                                    observed[row.capability],
                                  ),
                            ),
                          ),
                        ],
        ...
        // A device that cannot answer says so once, under the list, instead of
        // filling the column with dashes that mean nothing in particular.
        if (state?.unavailable case var why?) ...[
          const Gap(FwSpacing.xs),
          Text(
            why,
            style: context.type.caption.copyWith(color: context.colors.mut3),
        ...
        for (var note in state?.notes ?? const <String>[]) ...[
          const Gap(FwSpacing.xs),
          Text(
            note,
            style: context.type.caption.copyWith(color: context.colors.mut3),
...
/// Whether held and observed disagree in a way worth pointing at.
///
/// **Only when both are real answers.** `unknown` on either side means "this
/// side cannot tell", and the example's own adapter returns it for every
/// un-granted permission — `permission_handler` cannot separate *never asked*
/// from *refused*. Flagging that as a disagreement would light up every app in
/// `first-run`, which is the state the feature exists to make easy.
bool _disagree(HeldState? held, HeldState? observed) {
  if (held == null || observed == null) return false;
  if (held == HeldState.unknown || observed == HeldState.unknown) return false;
  return held != observed;
}

/// What the middle column says about a row.
///
/// Two different sentences depending on whether a device is chosen — see
/// [_PermissionsField.platform] for why the platform list stops being the
/// right one the moment it is.
class _RowSubject extends StatelessWidget {
  const _RowSubject({required this.row, this.platform});
  ...
  @override
  Widget build(BuildContext context) {
    var muted = context.type.caption.copyWith(color: context.colors.mut3);
    if (platform == null) {
      return Text(
        [for (var e in row.platforms) e.label].join(' · '),
    ...
    var mine = [
      for (var declaration in row.declarations)
        if (declaration.platform == platform) declaration.identifier,
    ];
    if (mine.isEmpty) {
      // The row exists because another platform declares it. On this device it
      // can never be granted, so the held column beside it will stay empty —
      // said here rather than left as an unexplained blank.
      return Text(
        'not on ${platform!.label}',
        textAlign: TextAlign.right,
        style: context.type.caption.copyWith(color: context.colors.amber),
      );
    }
    return Text(
      // The prefix is the same on every Android row and eats the width that
      // tells them apart; what is left is still what you would type after
      // `android.permission.`.
      mine.map((e) => e.replaceFirst('android.permission.', '')).join(', '),
      textAlign: TextAlign.right,
      overflow: TextOverflow.ellipsis,
      style: muted,
    );
...
/// One profile, as a thing to press.
///
/// Deliberately not a selected/unselected toggle: a profile is applied, not
/// held. What is true afterwards is the held column, read back from the
/// device — and it can differ from the profile the moment anything else
/// touches the app, which is exactly why the column and not the chip is the
/// source of truth.
class _ProfileChip extends StatelessWidget {
  ...
  /// Drawn before the label, for a chip that is not one of the profiles.
  final IconData? icon;
...
/// One cell of the held column.
///
/// The four states are not four shades of the same thing: `undetermined` is
/// the app's first run and reads as neutral, `granted` is working, and the two
/// denials are the ones somebody is usually hunting. `unknown` means this
/// store does not cover the row — kept visibly different from `undetermined`,
/// because one is an answer and the other is the absence of one.
class _HeldCell extends StatelessWidget {
  const _HeldCell({this.state, this.disagrees = false});

  final HeldState? state;

  /// Drawn as a problem rather than a state: the app believing something the
  /// OS does not is a bug in the app, and it is invisible from anywhere else.
  final bool disagrees;

  @override
  Widget build(BuildContext context) {
    var (text, color) = switch (state) {
      HeldState.granted => ('granted', context.colors.grn),
      HeldState.denied => ('denied', context.colors.amber),
      HeldState.deniedForever => ('denied forever', context.colors.red),
      HeldState.undetermined => ('will prompt', context.colors.mut2),
      HeldState.unknown => ('unknown', context.colors.mut3),
      null => ('—', context.colors.mut3),
    };
    return Text(
      text,
      textAlign: TextAlign.right,
      style: context.type.caption.copyWith(
        color: disagrees ? context.colors.red : color,
        fontWeight: disagrees ? FontWeight.w600 : null,
      ),
    );
  }
}

class _PermissionFinding extends StatelessWidget {
  const _PermissionFinding({required this.finding});

  final PermissionLint finding;

  @override
  Widget build(BuildContext context) {
    var (icon, color) = switch (finding.severity) {
      PermissionLintSeverity.problem => (
        Icons.error_outline,
        context.colors.red,
      ),
      PermissionLintSeverity.warning => (
        Icons.warning_amber_outlined,
        context.colors.amber,
      ),
      PermissionLintSeverity.note => (Icons.info_outline, context.colors.mut3),
    };
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
