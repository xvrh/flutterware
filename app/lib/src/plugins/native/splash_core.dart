...
class SplashCore extends PluginCore {
  ...
  Status get _status {
    ...
    if (scanned.isEmpty) {
      return _scanning.isEmpty
          ? const Status.neutral('Not computed')
    ...
    var broken = scanned.expand((s) => s.configErrors).length;
    if (broken > 0) {
      return Status.error(
        broken == 1
            ? scanned.expand((s) => s.configErrors).first
            : '$broken unusable config files',
      );
    }

    var configured = scanned.where((s) => s.isConfigured).length;
    if (configured == 0) return const Status.neutral('No splash configured');
      ...
      for (var config in scan.configs) {
        for (var problem in config.problems) {
          if (problem.blocksGeneration) {
            blocking++;
          } else if (problem.tone == Tone.warn) {
            warnings++;
          }
        }
      }
    ...
    if (blocking > 0) {
      return Status.error(
        '$blocking ${blocking == 1 ? 'problem' : 'problems'} stop generation',
      );
    }
    if (warnings > 0) {
      return Status.warn('$warnings to look at');
    }
    return const Status.good('Configured');
  }

  StatusBadge get _badge => switch (_status.tone) {
    Tone.error => const StatusBadge.dot(Tone.error),
    Tone.warn => const StatusBadge.dot(Tone.warn),
    _ => StatusBadge.none,
  };
