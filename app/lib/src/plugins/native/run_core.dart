...
import '../../run/native/adb_driver.dart';
...
import '../../run/permission_matrix.dart';
import '../../run/permission_memory.dart';
import '../../run/permission_state.dart';
import '../../run/permission_write.dart';
...
class RunCore extends PluginCore {
    ...
    return PluginReport(
      ...
      actions: [
        ...
        PluginAction(
          ...
          description:
              'What each package **declares** — the permissions in its '
              'AndroidManifest, the usage descriptions in its Info.plist, a '
              "Mac app's entitlements — grouped so one capability is one row "
              'across platforms, with what disagrees called out: background '
              'location with no foreground location, an always-on location key '
              'with no when-in-use key, a usage description left blank, a '
              'capability declared on one platform and not the other. Needs no '
              'device, no run and no build, because these are checked-in '
              'files. After a build it also reads the merged manifest, which '
              "is the only place a dependency's permissions appear — so the "
              'list grows once, and says which one it is showing.\n\n'
              'Pass `device` to add what the **OS currently holds** for the '
              'app on it — granted, denied, denied-forever or undetermined — '
              'read from `dumpsys package` on Android and from the '
              "simulator's own TCC database on iOS. Read-only. A device that "
              'cannot answer says why rather than reporting an empty list: '
              'macOS has no readable store, and iOS location lives outside '
              'the one this reads.',
          parameters: [
            _packageParameter,
            const ActionParameter(
              ...
              required: false,
              description:
                  'A device id from `devices` — an Android serial, or a booted '
                  "simulator's UDID. Omit for the declared list alone, which "
        ...
        PluginAction(
          ...
          danger: true,
          description:
              'Puts one capability into a state on a device: `granted`, '
              '`denied`, `deniedForever` or `undetermined`. **Answers with '
              'the state after the write, read back from the device** — never '
              'the request echoed, because `pm grant` of a permission the app '
              'never declared exits 0, prints nothing and grants nothing. A '
              'write that did not land says so.\n\n'
              'Android composes this from `pm grant`/`revoke` plus the '
              'permission flags, since revoke alone cannot express the '
              'difference between denied and never-asked. **A revoke kills '
              'the app and ends any `flutter run` attached to it** — measured. '
              'iOS has no separate denied-forever and says so rather than '
          ...
          parameters: [
            ...
            const ActionParameter(
              'device',
              'Device',
              description: 'The device to write to',
            ),
            const ActionParameter(
              'permission',
              'Permission',
              description:
                  'A capability id as `permissions` reports it — `camera`, '
            ...
            const ActionParameter(
              ...
              description: 'The state to put it in',
              options: [
                ActionOption('granted'),
                ActionOption('denied'),
                ActionOption('deniedForever', label: 'denied forever'),
                ActionOption('undetermined', label: 'undetermined'),
              ],
            ),
          ],
        ),
        PluginAction(
          ...
          danger: true,
          description:
              'Puts **every permission the app declares** into one state, and '
              'answers with the read-back. `first-run` is the one worth '
              'having: everything undetermined, so the app prompts as it does '
              'for a new user — the state that otherwise means uninstalling '
              'or tapping through Settings.\n\n'
              'Composed per permission rather than delegated to '
              '`pm reset-permissions`, which takes no package argument and '
          ...
          parameters: [
            ...
            ActionParameter(
              ...
              'Profile',
              kind: ActionParameterKind.choice,
              description: 'Which set of states to apply',
              options: [
                for (var profile in PermissionProfile.values)
                  ActionOption(profile.id, label: profile.label),
              ],
            ),
          ],
        ),
        PluginAction(
          'permissionMatrix',
          'Compare permission profiles',
          returns: RunPermissionMatrixResult,
          danger: true,
          description:
              'Runs the app once per permission profile and photographs the '
              'same screen from each — the answer to "what does this look '
              'like to a user who said no". Every cell carries what the OS '
              'held after the profile was applied, what the app itself '
              'reported, the screenshot, and the texts that appeared in it '
              'and not in the first cell.\n\n'
              '**Each cell is a relaunch, and this is minutes rather than '
              'seconds.** Permission state is read at process start and a '
              'revoke kills the app anyway, so there is no cheaper way to '
              'see it. The first failed launch stops the run: a build that '
              'cannot compile will not compile three more times.\n\n'
              'Pass `route` to drive each launch somewhere before the '
              'picture is taken — the first screen of an app is usually not '
              'where a permission is read, and a matrix of four identical '
              'screenshots says so rather than pretending.',
          parameters: [
            _packageParameter,
            ActionParameter(
              'device',
              'Device',
              kind: ActionParameterKind.choice,
              description: 'Which device to run the whole matrix on',
              options: [
                for (var device in devices)
            ...
            ActionParameter(
              ...
              required: false,
              description:
                  'Which profiles to run, comma-separated — '
                  '`first-run,granted`. All of them when omitted, in the '
                  'order given when not: the first one is what the others are '
                  'compared against. An unknown name is refused rather than '
                  'skipped.\nDeclared: '
                  '${[for (var e in PermissionProfile.values) e.id].join(', ')}.',
            ),
            const ActionParameter(
              'route',
              'Route',
              required: false,
              description:
                  'A route to drive each launch to before it is '
                  "photographed, as the app's navigation handler understands "
                  'it. Without one every cell shows the first screen.',
            ),
            const ActionParameter(
              'flavor',
              'Flavor',
              required: false,
              description:
                  'The `--flavor` to build every cell with. Defaults to what '
            ...
            const ActionParameter(
              ...
              required: false,
              description:
                  'The values every cell passes its entry point, '
                  '`name=value,name=value` or a JSON object. The same for all '
                  'of them — this varies permissions, nothing else.',
            ),
            const ActionParameter(
              'dartDefines',
              'Dart defines',
              required: false,
              description:
                  'The `--dart-define`s every cell is built with, same escape '
                  'hatch and same caveats as on `launch`.',
            ),
            const ActionParameter(
              'timeout',
              'Timeout',
              ...
              required: false,
              defaultValue: '300',
              description: 'Seconds to wait for each launch',
            ),
          ],
        ),
        PluginAction(
          ...
          parameters: [
            ...
            ActionParameter(
              ...
              required: false,
              description:
                  'A permission profile to put the app in **before it '
                  'starts** — the robust point, since nothing is running yet '
                  'and an app that asks on its first frame gets what you '
                  'asked for. Sticky: it applies to every later launch until '
                  'you clear it with an empty value, and each launch says so. '
                  'Needs the app installed once first on Android; on the iOS '
                  'simulator it works even before the first install.',
              options: [
                for (var profile in PermissionProfile.values)
                  ActionOption(profile.id, label: profile.label),
              ],
  ...
  Future<Object?> invoke(
    ...
    return switch (actionId) {
      ...
      'permissions' => _permissionsAction(
        arguments['package'] as String?,
        arguments['device'] as String?,
      ),
      'permissionSet' => _permissionWriteAction(arguments),
      'permissionProfile' => _permissionWriteAction(arguments),
      'permissionMatrix' => _permissionMatrixAction(arguments),
  ...
  /// The declared third of every permission, per package.
  ///
  /// Costs a handful of file reads and nothing else — no `computeAll`, no
  /// process, no device — which is what lets this answer before a project has
  /// ever been built or run.
  Future<RunPermissionsResult> _permissionsAction(
    String? package,
    String? device,
  ) async {
    ...
    var reports = [
      for (var path in wanted)
        (path, readDeclarations(host.workspace.absolutePathOf(path))),
    ];
    var held = <String, HeldPermissions>{};
    var observed = <String, Map<String, HeldState>>{};
    if (device != null) {
      for (var (path, _) in reports) {
        held[path] = await _heldFor(
          host.workspace.absolutePathOf(path),
          device,
        );
      }
      // The app's own view, when there is an app running on that device with
      // an adapter wired. Read once — one device runs one app — but filed
      // under **that app's package only**. Held is a property of the pair
      // (device, package) and every package gets its own answer; observed is a
      // property of one *process*, and putting it on every package's rows
      // published one app's belief under a sibling's name, where the cockpit
      // then compared it against the sibling's held state.
      var (running, states) = await _observedWithPackage(device);
      if (running != null) observed[running] = states;
    }
    return RunPermissionsResult(
      packages: [
        for (var (path, report) in reports)
          _permissionsPackage(
            path: path,
            declared: report,
            device: device,
            held: held[path],
            observed: observed[path] ?? const {},
          ),
      ],
    );
  }

  /// The permission memory for this machine.
  late final permissionMemory = PermissionMemory(runDirProvider());

  /// Records [requested] as the wish when given, then applies whatever wish is
  /// in force to [device]. Returns a line for the launch reply, or null.
  ///
  /// Silent when there is no wish, which is the common case — a launch that
  /// says nothing about permissions is a launch nobody configured.
  Future<String?> _applyPermissionWish({
    required String package,
    required String device,
    String? requested,
  }) async {
    var key = PermissionMemory.keyFor(host.worktree.path, package);
    if (requested != null) {
      if (requested.isEmpty) {
        permissionMemory.clear(key);
      } else if (PermissionProfile.byId(requested) == null) {
        return 'Ignored permissions="$requested": no such profile. Declared: '
            '${[for (var e in PermissionProfile.values) e.id].join(', ')}.';
      } else {
        permissionMemory.write(key, PermissionWish(profile: requested));
      }
    }
    var wish = permissionMemory.read(key);
    var profile = wish.resolved;
    if (profile == null) return null;

    var root = host.workspace.absolutePathOf(package);
    var declared = readDeclarations(root);
    var wanted = profileTargets(
      profile,
      declared.rows,
      on: await _writePlatformFor(device),
    );
    if (wanted.isEmpty) return null;
    var isSimulator = Platform.isMacOS && await _isBootedSimulator(device);
    var written = await writePermissions(
      device: device,
      packageRoot: root,
      wanted: wanted,
      isSimulator: isSimulator,
      readBack: () => _heldFor(root, device),
    );
    // Said on every launch, not only the one that set it. A wish you have
    // forgotten is the debugging hour FlagMemory's docstring is about.
    return _joinNotes([
      'Permissions: "${profile.id}" applied before launch.',
      written.unavailable,
      ...written.refused,
    ]);
  }

  /// Both write actions, which differ only in how the target states are built.
  ///
  /// One handler because the *answer* is the same shape and the same promise:
  /// the read-back. A caller that set one permission and a caller that applied
  /// a whole profile both need to be told what is true now, not what they
  /// asked for.
  Future<RunPermissionsResult> _permissionWriteAction(
    Map<String, Object?> arguments,
  ) async {
    var package = arguments['package'] as String?;
    var device = arguments['device'] as String?;
    if (device == null || device.isEmpty) {
      return RunPermissionsResult(
        packages: const [],
        note: 'A device is required to write to. `devices` lists them.',
      );
    }
    var path = package ?? (packages.length == 1 ? packages.single : null);
    if (path == null) {
      return RunPermissionsResult(
        packages: const [],
        note: packages.isEmpty
            ? 'No packages are declared for this plugin.'
            : 'This project declares more than one package, so `package` is '
                  'required: ${packages.join(', ')}.',
      );
    }
    var root = host.workspace.absolutePathOf(path);
    var declared = readDeclarations(root);

    Map<String, HeldState> wanted;
    if (arguments['profile'] case var id?) {
      var profile = PermissionProfile.byId('$id');
      if (profile == null) {
        return RunPermissionsResult(
          packages: const [],
          note:
              'No profile "$id". Declared: '
              '${[for (var e in PermissionProfile.values) e.id].join(', ')}.',
        );
      }
      var platform = await _writePlatformFor(device);
      wanted = profileTargets(profile, declared.rows, on: platform);
      if (wanted.isEmpty) {
        return RunPermissionsResult(
          packages: const [],
          note:
              'This app declares no permissions a profile could set '
              '${platform == null ? '' : 'on ${platform.label} '}'
              'so "${profile.id}" would do nothing.',
        );
      }
    } else {
      var permission = '${arguments['permission'] ?? ''}';
      var value = '${arguments['value'] ?? ''}';
      var state = HeldState.values.where((e) => e.name == value).firstOrNull;
      if (permission.isEmpty || state == null) {
        return RunPermissionsResult(
          packages: const [],
          note:
              '`permission` and `value` are both required. Value is one of '
              'granted, denied, deniedForever, undetermined.',
        );
      }
      // **Refused before the write, not diagnosed after.** `pm grant` of a
      // permission the app never declared exits 0, prints nothing and grants
      // nothing (measured, S-P1) — and the read-back cannot expose that,
      // because a capability the app does not declare has no row to be wrong
      // in. Asked for microphone on an app that never declared it, the first
      // version of this ran four adb commands and reported success.
      var declaredIds = {
        for (var row in declared.rows)
          if (row.known) row.capability,
      };
      if (!declaredIds.contains(permission)) {
        return RunPermissionsResult(
          packages: const [],
          note:
              'This app does not declare $permission, so setting it would do '
              'nothing — Android accepts the command and ignores it. Declare '
              'it in the manifest first. Declared here: '
              '${declaredIds.isEmpty ? 'nothing' : declaredIds.join(', ')}.',
        );
      }
      wanted = {permission: state};
    }
    ...
    return _permissionsReply(
      path: path,
      declared: declared,
      device: device,
      held: written.after,
      // Everything the platform could not do, said out loud. A request that
      // quietly became something else is the failure this design exists to
      // prevent.
      extraNote: [
        ?written.unavailable,
        ...written.refused,
        for (var step in written.steps)
          if (!step.ok) 'Failed: ${step.command} — ${step.error}',
      ].join(' ').trim(),
    );
  }

  /// Applies a profile and records it as the wish, for the launch form.
  ///
  /// The same path the action takes, so the button and the agent cannot drift
  /// into doing different things.
  Future<PermissionWriteResult> applyPermissionProfile({
    required String package,
    required String device,
    required PermissionProfile profile,
  }) async {
    var root = host.workspace.absolutePathOf(package);
    var declared = readDeclarations(root);
    permissionMemory.write(
      PermissionMemory.keyFor(host.worktree.path, package),
      PermissionWish(profile: profile.id),
    );
    var isSimulator = Platform.isMacOS && await _isBootedSimulator(device);
    return writePermissions(
      device: device,
      packageRoot: root,
      wanted: profileTargets(
        profile,
        declared.rows,
        on: await _writePlatformFor(device),
      ),
  ...
  /// The wish in force for [package], so the form can show it as wished.
  PermissionWish permissionWishFor(String package) => permissionMemory.read(
    PermissionMemory.keyFor(host.worktree.path, package),
  );

  Future<RunPermissionMatrixResult> _permissionMatrixAction(
    Map<String, Object?> arguments,
  ) async {
    await computeAll();
    var device = arguments['device'] as String?;
    ...
    var (:profiles, :error) = parseMatrixProfiles(arguments['profiles']);
    if (error != null) {
      return RunPermissionMatrixResult(
        device: device,
        package: package,
    ...
    return permissionMatrix(
      ...
      entry: entry,
      profiles: profiles,
      route: switch (arguments['route']) {
        String given when given.isNotEmpty => given,
        _ => null,
      },
      ...
      knobs: knobs,
      defines: PreviewsCore.parsePairs(arguments['dartDefines']),
      timeout: Duration(seconds: _intArgument(arguments['timeout'], 300)),
    );
  }

  /// Runs [entry] once per profile and photographs the same screen from each.
  ///
  /// The panel's entry point too, so the dialog and the action cannot drift
  /// into running different matrices.
  ///
  /// Three things about the order, each of which was a choice:
  ///
  /// - **Stop, apply, launch.** Applying to a running app is the case S-P1
  ///   measured killing it; applying to a stopped one costs nothing and is
  ///   read at the next process start, which is the launch this then does.
  /// - **`launch` rather than the launch *action*.** The action applies the
  ///   sticky wish, which would stamp the same profile over every cell. A
  ///   matrix is a sweep, not a configuration — it leaves the wish alone, and
  ///   says so when there is one.
  /// - **Held is read after the picture, not from the write.** The cell's
  ///   column then describes the same moment the screenshot does, including
  ///   anything the app itself changed by prompting on its first frame.
  Future<RunPermissionMatrixResult> permissionMatrix({
    required String device,
    required String package,
    required EntrypointRef entry,
    required List<PermissionProfile> profiles,
    String? route,
    String? flavor,
    Map<String, String> knobs = const {},
    Map<String, String> defines = const {},
    Duration timeout = const Duration(seconds: 300),
    void Function(String) onProgress = _ignoreProgress,
  }) async {
    var root = host.workspace.absolutePathOf(package);
    var declared = readDeclarations(root);
    var platform = await _writePlatformFor(device);
    if (profileTargets(profiles.first, declared.rows, on: platform).isEmpty) {
      return RunPermissionMatrixResult(
        device: device,
        package: package,
        entrypoint: entry.path,
        ...
        note:
            'This app declares no permissions a profile could set '
            '${platform == null ? '' : 'on ${platform.label} '}'
            'so every cell would be the same launch.',
      );
    }
    var isSimulator = Platform.isMacOS && await _isBootedSimulator(device);
    var cells = <RunPermissionMatrixCell>[];
    List<String>? baseline;
    // Separate from `baseline` being non-null, because **the first cell is the
    // baseline even when it has no texts.** With `baseline ??= …` a first cell
    // whose observe came back empty let the second one quietly become the
    // reference, while the grid went on labelling the first "compared
    // against" — every later cell's `added` was then measured from somewhere
    ...
    for (var profile in profiles) {
      ...
      var step = '${profile.label} (${cells.length + 1}/${profiles.length})';

      onProgress('$step — stopping what is running');
      await _stopEverythingOn(device);

      onProgress('$step — applying');
      var written = await writePermissions(
        device: device,
        packageRoot: root,
        wanted: profileTargets(profile, declared.rows, on: platform),
      ...
      var writeNote = _joinNotes([
        written.unavailable,
        ...written.refused,
        for (var s in written.steps)
          if (!s.ok) 'Failed: ${s.command} — ${s.error}',
      ]);

      onProgress('$step — launching');
      RunHandle handle;
      LaunchLog log;
      try {
        handle = await launch(
        ...
        (handle, log) = await awaitLaunch(
          handle,
          timeout,
          onProgress: (line) => onProgress('$step — $line'),
        );
      } on Object catch (e) {
        cells.add(
          RunPermissionMatrixCell(
            profile: profile.id,
            label: profile.label,
            ok: false,
            error: '$e',
        ...
        cutShort = 'Stopped after "${profile.id}": the launch itself failed.';
        break;
      }
      _handles = _scanHandles();
      await _probeAll();
      ...
      if (failure != null || (log.stopped && !log.started)) {
        ...
        cells.add(
          RunPermissionMatrixCell(
            ...
            label: profile.label,
            ok: false,
            error: failure ?? 'the app stopped starting',
            note: writeNote,
            ms: elapsed(),
          ),
        );
        // **The first failure ends the run.** What fails a launch here is
        // nearly always the build or the device, and neither gets better by
        // being asked three more times at a minute and a half each.
        cutShort =
            'Stopped after "${profile.id}" — it did not come up, and the '
            'profiles left would have failed the same way. '
            '${handle.logPath == null ? '' : 'The log is ${handle.logPath}.'}';
        break;
      }

      onProgress('$step — observing');
      var act = await _observeWhenReady({
        'device': device,
        'entrypoint': entry.path,
        'verb': route == null ? 'observe' : 'navigate',
        'route': ?route,
        'actor': 'matrix',
      });
      // Held **after** the picture, so the column and the screenshot describe
      // one moment — see the doc above.
      var held = await _heldFor(root, device);
      var observed = await observedPermissions(
        device: device,
        entrypoint: entry.path,
      );
      if (!haveBaseline) {
      ...
      cells.add(
        RunPermissionMatrixCell(
          ...
          label: profile.label,
          ok: true,
          held: {for (var e in held.byCapability.entries) e.key: e.value.name},
          observed: {for (var e in observed.entries) e.key: e.value.name},
          screenshot: act.screenshot,
          screenshotArtifact: act.screenshotArtifact,
          texts: act.texts,
          added: cells.isEmpty ? const [] : textsAdded(baseline, act.texts),
          // A refused navigate is not a failed cell — the app came up and was
          // photographed, just not where it was asked to go. Saying that is
          // worth more than throwing the launch away.
          note: _joinNotes([
            writeNote,
            held.unavailable,
            ...held.notes,
            if (!act.ok) 'The ${act.verb} was refused: ${act.error}',
          ]),
          ms: elapsed(),
        ),
      );
    ...
    // The wish outlives the matrix and will quietly undo its last cell at the
    // next ordinary launch. Better said now than discovered then.
    var wish = permissionWishFor(package).resolved;
    var wishNote = wish == null
        ? null
        : 'A permission wish ("${wish.id}") is in force for this app, so the '
              'next ordinary launch re-applies it.';
    return RunPermissionMatrixResult(
      device: device,
      package: package,
      entrypoint: entry.path,
      ...
      note: _joinNotes([
        cutShort,
        sameScreenNote([for (var cell in cells) cell.texts]),
        if (cells.isNotEmpty && cells.last.ok)
          'Left running under "${cells.last.profile}".',
        wishNote,
      ]),
    );
  }

  static void _ignoreProgress(String _) {}

  /// Observes, retrying until there is a picture.
  ///
  /// **`awaitLaunch` returning is not the app being ready to look at.** It
  /// answers when the tool says the app started, which is before the drive
  /// guest has registered its extension and before the first frame has been
  /// laid out. Measured on the emulator, the first matrix that ran: two of
  /// four cells came back empty, in the two distinct ways a too-early look
  /// fails — *"this app is running without the drive guest"* and
  /// *"Invalid image dimensions"* — and the two cells that did work were the
  /// slower ones. Nothing distinguishes either from a real refusal except
  /// trying again.
  ///
  /// **The picture is the condition, not `ok`.** A verb that is genuinely
  /// refused — a navigate at an app with no handler — still comes back with
  /// the screen it was refused on, and that cell is finished: retrying it
  /// would spend six seconds re-reading the same refusal. No picture is the
  /// one failure that is worth waiting out.
  Future<RunActResult> _observeWhenReady(
    Map<String, Object?> arguments, {
    int tries = 8,
    Duration between = const Duration(seconds: 1),
  }) async {
    for (var attempt = 1; ; attempt++) {
      var result = await _actAction(arguments);
      if (result.screenshot != null || attempt >= tries) return result;
      await Future<void>.delayed(between);
    }
  }

  /// Stops every run *this worktree* has on [device].
  ///
  /// By device rather than by entry point: a phone runs one app, and a stale
  /// process there answers the observe with the previous cell's screen.
  /// Other worktrees' runs are left alone — stopping somebody else's app
  /// because they share a phone is not a thing this will do; the launch below
  /// will report the clash the way every other launch does.
  Future<void> _stopEverythingOn(String device) async {
    _handles = _scanHandles();
    for (var handle in [
      for (var handle in ownHandles)
        if (handle.device == device) handle,
    ]) {
      try {
        await control('stop', handle);
      } on Object {
        // A run that will not stop is not a reason to abandon the matrix: the
        // launch below reports a busy device far better than this could.
      }
    }
  }

  /// What the app running on [device] believes about its own permissions.
  ///
  /// Empty for an app with no `PermissionAdapter` wired, and empty is the
  /// right answer there — this third is the app's to offer, and most apps do
  /// not. It is never an error: a cockpit that failed because an app declined
  /// to describe itself would be unusable on every app but one.
  Future<Map<String, HeldState>> observedPermissions({
    required String device,
    String? entrypoint,
  }) async {
    try {
      return await _readObserved(
        await _selectRunningApp({'device': device, 'entrypoint': ?entrypoint}),
      );
    } on Object {
      // No app running on that device, no channel, no panel — all of which are
      // ordinary, and none of which is worth failing a read over.
      return const {};
    }
  }

  /// The same reading, with **which package's app produced it**.
  ///
  /// The pair rather than the map, because a caller reporting on several
  /// packages has to file this under one of them. One selection, not one per
  /// package: [_selectRunningApp] probes every handle, and asking it N times
  /// to answer a question about a single process is N-1 round trips too many.
  ///
  /// Null when nothing is running there, when the handle predates the
  /// `package` field, or when the app belongs to no package this plugin
  /// declares — and then no package gets an observed column, which is the
  /// honest answer rather than a guess at whose it was.
  Future<(String?, Map<String, HeldState>)> _observedWithPackage(
    String device,
  ) async {
    try {
      var handle = await _selectRunningApp({'device': device});
      return (handle.package, await _readObserved(handle));
    } on Object {
      return (null, const <String, HeldState>{});
    }
  }

  Future<Map<String, HeldState>> _readObserved(RunHandle handle) =>
      _withPanels(handle, (client, panels) async {
        var declared = await panels.list();
        if (!declared.any((e) => e.id == permissionsPanelId)) return const {};
        return parseObservedPermissions(
          await panels.state(permissionsPanelId, permissionsStatusStateId),
        );
      });

  /// What [device] holds for [package], for a caller that has a package path
  /// rather than a root — the launch form.
  Future<HeldPermissions> heldPermissions({
    required String package,
    required String device,
  }) => _heldFor(host.workspace.absolutePathOf(package), device);

  /// What [device] holds for the app [packageRoot] builds.
  ///
  /// The dispatch is by *what the device is*, asked of the tools rather than
  /// guessed from the shape of the id — the same rule `NativeSession` follows,
  /// and for the same reason: a physical iOS serial looks like nothing in
  /// particular. Everything that cannot answer says why, because the spikes
  /// found three separate ways for a permission read to come back empty and
  /// mean three different things.
  Future<HeldPermissions> _heldFor(String packageRoot, String device) async {
    if (AdbNativeDriver.findAdb() case var adb?
        when await AdbNativeDriver.owns(device, adb)) {
      var identity = androidIdentity(packageRoot);
      if (identity == null) {
        return const HeldPermissions(
          unavailable:
              "Could not work out this app's application id. Build it once, "
              'or set applicationId in android/app/build.gradle.',
        );
      }
      return readAndroidHeld(serial: device, identity: identity, adbPath: adb);
    }
    if (device == 'macos') {
      return const HeldPermissions(
        unavailable:
            'macOS keeps its permissions in a TCC database no process can '
            'read without Full Disk Access, and tccutil reports success '
            'whether or not anything changed — so there is nothing here that '
            'could be trusted. Declared and observed only on this platform.',
      );
    }
    if (Platform.isMacOS && await _isBootedSimulator(device)) {
      ...
      return readSimulatorHeld(udid: device, identity: identity);
    }
    // Named for what it actually is. A catch-all that led with the iPhone case
    // told somebody asking about Chrome that physical iPhones have no store,
    // which is true and not an answer to the question they asked.
    var kind = await _deviceKind(device);
    return HeldPermissions(
      unavailable: switch (kind) {
        'ios' =>
          'A physical iPhone has no host-side permission store to read. Only '
              'the app itself can report what it sees — that is what the '
              'devbar adapter is for.',
        'web' =>
          'A browser has no per-app permission store on this machine. Web '
              'permissions live in the browser profile and are reachable over '
              'the DevTools protocol, which this does not speak yet.',
        _ =>
          'Nothing on this machine can read what "$device" holds. It is not '
              'an Android device adb knows, not this Mac, and not a booted '
              'simulator.',
      },
    );
  }

  /// Which platform a write to [device] lands on, for narrowing a profile.
  ///
  /// From the cached device list, so it costs nothing — and **null when the
  /// list cannot say**, which deliberately means "do not narrow". A device
  /// this cannot name is a device whose platform nothing here knows, and
  /// silently dropping capabilities on that guess would be the quiet version
  /// of the bug this exists to fix.
  Future<PermissionPlatform?> _writePlatformFor(String device) async =>
      switch (await _deviceKind(device)) {
        'android' => PermissionPlatform.android,
        'ios' => PermissionPlatform.ios,
        'macos' => PermissionPlatform.macos,
        _ => null,
      };

  /// A coarse family for [device], from the cached device list rather than
  /// from the shape of the id.
  ///
  /// The cache rather than a fresh daemon: this only ever decorates a refusal,
  /// and a refusal is not worth a few seconds of `flutter daemon`.
  Future<String?> _deviceKind(String device) async {
    try {
      _cache ??= DeviceCache.read(runDirProvider());
      for (var entry in devices) {
        if (entry.id == device) return entry.platformType;
      }
    } on Object {
      // A device list this cannot read is not worth failing a refusal over;
      // the generic wording below is still true.
    }
    return null;
  }

  /// Whether [device] is a booted simulator, asked of `simctl`.
  Future<bool> _isBootedSimulator(String device) async {
    try {
      ...
      if (jsonDecode('${result.stdout}') case {'devices': Map runtimes}) {
        for (var runtime in runtimes.values) {
          for (var entry in (runtime as List).cast<Map<String, Object?>>()) {
            if (entry['udid'] == device) return true;
          }
        }
      }
    } on Object {
      // No Xcode, or an answer this does not understand. Either way it is not
      // a simulator as far as this can tell.
    }
    return false;
  }

  /// One package's reply, shared by the read and the two writes.
  ///
  /// Shared deliberately: a write answers with exactly what a read answers,
  /// because the promise is that the reply describes the device rather than
  /// the request. Two builders would eventually drift into two promises.
  RunPermissionPackage _permissionsPackage({
    required String path,
    required PermissionDeclarations declared,
    String? device,
    HeldPermissions? held,
    Map<String, HeldState> observed = const {},
    String? extraNote,
  }) => RunPermissionPackage(
    path: path,
    platforms: [for (var e in declared.platformsPresent) e.name],
    merged: declared.merged,
    sources: declared.sources,
    device: device,
    appId: held?.identity?.id,
    appIdSource: held?.identity?.source,
    heldUnavailable: held?.unavailable,
    heldNotes: held?.notes ?? const [],
    rows: [
      for (var row in declared.rows)
        RunPermissionRow(
          capability: row.capability,
          label: row.label,
          platforms: [for (var e in row.platforms) e.name],
          known: row.known,
          held: held?.byCapability[row.capability]?.name,
          observed: observed[row.capability]?.name,
          declarations: [
            for (var declaration in row.declarations)
              RunPermissionDeclaration(
                identifier: declaration.identifier,
                platform: declaration.platform.name,
                source: declaration.source,
                usage: declaration.usage,
                maxSdkVersion: declaration.maxSdkVersion,
                fromDependency: declaration.fromDependency,
              ),
          ],
        ),
    ],
    findings: [
      for (var lint in declared.lints)
        RunPermissionFinding(
          id: lint.id,
          severity: lint.severity.name,
          message: lint.message,
          capability: lint.capability,
          platform: lint.platform?.name,
        ),
    ],
    note: _joinNotes([extraNote, _permissionNote(declared)]),
  );
  ...
  RunPermissionsResult _permissionsReply({
    required String path,
    required PermissionDeclarations declared,
    required String device,
    required HeldPermissions held,
    String? extraNote,
  }) => RunPermissionsResult(
    packages: [
      _permissionsPackage(
        path: path,
        declared: declared,
  ...
  /// What to say about a list before anybody misreads it.
  ///
  /// An empty list has two very different causes and they must not read the
  /// same: a package with no mobile platforms has nothing to declare, while a
  /// package that has them and declares nothing is a real, if unusual, answer.
  static String? _permissionNote(PermissionDeclarations report) {
    if (report.platformsPresent.isEmpty) {
      return 'No android/, ios/ or macos/ directory — this package has no '
          'platform that asks for permissions.';
    }
    if (report.isEmpty) {
      return 'Declares no permissions on '
          '${report.platformsPresent.map((e) => e.label).join(', ')}.';
    }
    if (!report.merged &&
        report.platformsPresent.contains(PermissionPlatform.android)) {
      return "This is the app's own list. Build it once and the merged "
          'manifest adds whatever its dependencies ask for.';
    }
    return null;
  }
  ...
  Future<RunLaunchResult> _launchAction(Map<String, Object?> arguments) async {
    ...
    // **Before the build starts, not after.** Pre-launch is the robust point
    // of intervention: nothing is running, so none of the commands that kill
    // an app can cost anything, and an app that asks for a permission on its
    // first frame gets the state that was asked for. The wish is applied on
    // every launch, not only the one that set it — that is what makes it a
    // configuration rather than a command.
    var permissionNote = await _applyPermissionWish(
      package: package,
      device: device,
      requested: arguments['permissions'] as String?,
    );
