...
/// over six trials of one clip it rendered a *different* clip from an
/// identical walk in four of them, frames offset by a stop, because it
...
///
/// The fixture is this app's own `Scene clip` demo: a scene and its motion
/// mounted with `SceneView`, which is the exact shape `fw run scene video`
/// walks. It was two motion-plugin demos until that plugin was deleted; the
/// second one, a flow that moved a `PageView` from a post-frame callback and
/// only landed its playhead on a later frame, has no scene counterpart —
/// `SceneView` applies its playhead in the frame that seeks it.
void main() {
  var fixtures = {'a scene': 'tool/catalog/demos/scene_clip.dart#sceneClip'};
