...
void main() {
  ...
  test(
    'installMirror builds a facade: real bin/, symlinked entries, shims',
    () {
      ...
      expect(
        FileSystemEntity.isLinkSync(p.join(mirror.path, 'pubspec.yaml')),
        isTrue,
      );
      // bin/ is a real directory.
      expect(
        FileSystemEntity.isDirectorySync(p.join(mirror.path, 'bin')),
        isTrue,
      );
      expect(FileSystemEntity.isLinkSync(p.join(mirror.path, 'bin')), isFalse);
      // bin/cache and bin/internal are symlinks.
      expect(
        FileSystemEntity.isLinkSync(p.join(mirror.path, 'bin', 'cache')),
        isTrue,
      );
      ...
      expect(
        File(p.join(mirror.path, 'bin', 'dart')).readAsStringSync(),
        contains('KIND="dart"'),
      );
    },
  );
