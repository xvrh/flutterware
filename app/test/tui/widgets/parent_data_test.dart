...
void main() {
  test(
    'Expanded writes flex and a tight fit into the child FlexParentData',
    () {
      var binding = pumpHosted(
        const Row(children: [Expanded(flex: 2, child: Text('x'))]),
      );
  ...
  test('Flexible writes flex and a loose fit', () {
    var binding = pumpHosted(
      const Row(children: [Flexible(flex: 3, child: Text('x'))]),
    );
    var pd = onlyChildParentData(binding);
    expect(pd.flex, 3);
  ...
  test('rebuilding with a new flex updates the FlexParentData', () {
    ...
    rebuild(
      binding,
      const Row(children: [Expanded(flex: 5, child: Text('x'))]),
    );
  ...
  test(
    'changing the Expanded child to a different type still applies flex',
    () {
      ...
      expect(
        (child.parentData! as FlexParentData).flex,
        2,
  ...
  test('switching Expanded to Flexible changes the fit', () {
    var binding = pumpHosted(
      const Row(children: [Expanded(flex: 1, child: Text('x'))]),
    );
    expect(onlyChildParentData(binding).fit, FlexFit.tight);

    rebuild(
      binding,
      const Row(children: [Flexible(flex: 1, child: Text('x'))]),
    );
  ...
  test('Expanded flex actually drives layout', () {
    ...
    var children = (binding.renderView.child! as RenderFlex).children
        .cast<RenderText>();
