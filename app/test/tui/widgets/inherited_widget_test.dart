...
TuiBinding pump(Widget body) {
  ...
  binding.handleResize(CellSize(8, 16));
  binding.drawFrame(Painter(CellBuffer(8, 16)));
  return binding;
}
...
void main() {
  ...
  test('dependOnInheritedWidgetOfExactType returns the nearest ancestor', () {
    pump(const Model(value: 7, child: Column(children: [Dependent('d')])));
    expect(buildLog, ['d=7']);
  });
  ...
  test('updateShouldNotify == true rebuilds exactly the dependents', () {
    var binding = pump(const Model(
        value: 1, child: Column(children: [Dependent('a'), Dependent('b')])));
  ...
  test('updateShouldNotify == false skips dependent rebuilds', () {
    ...
    rebuild(
        ...
        const Model(
            value: 2,
            notify: false,
            child: Column(children: [Dependent('a')])));
