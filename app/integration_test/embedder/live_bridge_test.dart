...
void main() {
  test(
    'guest streams live frames and re-allocates surfaces on resize',
    () async {
      ...
      var socketPath = checkSocketPath(
        p.join(flutterwareRunDir(), 'embt-$pid.sock'),
      );
      ...
      Future<T> next<T extends EmbedderMessage>() async {
        while (true) {
          var message = await messages.next.timeout(
